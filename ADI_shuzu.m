...
j=ny1+1:ny2;   k=nz1:nz2;  i=nx1+1:nx2;
d_z1(i-1,j,k)=CA*Ez1(i,j,k)-CQ*CB/dx/dz*(Ex1(i,j,k+1)-Ex1(i,j,k)-Ex1(i-1,j,k+1)+Ex1(i-1,j,k))...
            -CB/dy*(Hx1(i,j,k)-Hx1(i,j-1,k))+CB/dx*(Hy1(i,j,k)-Hy1(i-1,j,k));
d_z1(1,j,k)=d_z1(1,j,k)-a_x*dx/(dx+c0*dt)*(Ez1(1,j,k)+Ez1(2,j,k));
d_z1(nx2-1,j,k)=d_z1(nx2-1,j,k)-c_x*dx/(dx+c0*dt)*(Ez1(nx2+1,j,k)+Ez1(nx2,j,k));
     
for j=ny1+1:ny2
    for k=nz1:nz2     
        i=nx1+1:nx2;        
        Ez(i,j,k)=JAx\d_z1(i-1,j,k);
    end
end
% k=nx1:nx2-1;
% i=ny1+1:ny2; 
...
i=nx1:nx2;  k=nz1+1:nz2  ;j= ny1+1:ny2;
d_x1(i,j-1,k)=CA*Ex1(i,j,k)-CQ*CB/dx/dy*(Ey1(i+1,j,k)-Ey1(i,j,k)-Ey1(i+1,j-1,k)+Ey1(i,j-1,k))...
            -CB/dz*(Hy1(i,j,k)-Hy1(i,j,k-1))+CB/dy*(Hz1(i,j,k)-Hz1(i,j-1,k));  
        
d_x1(i,1,k)=d_x1(i,1,k)- a_y *  dy/(dy+c0*dt) *( Ex1(i,1,k) + Ex1(i,2,k) );
d_x1(i,ny2-1,k)=d_x1(i,ny2-1,k)- c_y *  dy/(dy+c0*dt) *(Ex1(i,ny2+1,k) + Ex1(i,ny2,k) );


for i=nx1:nx2
    for k=nz1+1:nz2
        j= ny1+1:ny2;         
        Ex(i,j,k)=JAy\d_x1(i,j-1,k)';     
    end 
end
% i=nx1:nx2-1;
% j=ny1+1:ny2; 
...
i=nx1+1:nx2;  j=ny1:ny2;  k=nz1+1:nz2;
d_y1(i,j,k-1)=CA*Ey1(i,j,k)-CQ*CB/dy/dz*(Ez1(i,j+1,k)-Ez1(i,j,k)-Ez1(i,j+1,k-1)+Ez1(i,j,k-1))...
            -CB/dx*(Hz1(i,j,k)-Hz1(i-1,j,k))+CB/dz*(Hx1(i,j,k)-Hx1(i,j,k-1));
d_y1(i,j,1)=d_y1(i,j,1)-a_z*+dz/(dz+c0*dt)*(Ey1(i,j,1)+ Ey1(i,j,2));
d_y1(i,j,nz2-1)=d_y1(i,j,nz2-1)-c_z*+dz/(dz+c0*dt)*(Ey1(i,j,nz2+1)+ Ey1(i,j,nz2));

for i=nx1+1:nx2
    for j=ny1:ny2
        k=nz1+1:nz2;
        Ey(i,j,k)=JAz\reshape(d_y1(i,j,:),nz2-1,1);        
    end
end
% k=nz1+1:nz2;
% j=ny1:ny2-1;
...
j=ny1:ny2; k=nz1+1:nz2; i=nx1+1:nx2;
d_y2(i-1,j,k)=Ey1(i,j,k)-CQ*CB/dx/dy*(Ex1(i,j+1,k)-Ex1(i,j,k)-Ex1(i-1,j+1,k)+Ex1(i-1,j,k))...
            -CB/dx*(Hz1(i,j,k)-Hz1(i-1,j,k))+CB/dz*(Hx1(i,j,k)-Hx1(i,j,k-1));
d_y2(1,j,k)=d_y2(1,j,k)-a_x*dx/(dx+c0*dt)*(Ey1(1,j,k)+Ey1(2,j,k));
d_y2(nx2-1,j,k)=d_y2(nx2-1,j,k)-c_x*dx/(dx+c0*dt)*(Ey1(nx2+1,j,k)+Ey1(nx2,j,k));
...
%----------ez---------------  
i=nx1+1:nx2;  k=nz1:nz2;  j=ny1+1:ny2;
d_z2(i,j-1,k)=Ez1(i,j,k)-CQ*CB/dy/dz*(Ey1(i,j,k+1)-Ey1(i,j,k)-Ey1(i,j-1,k+1)+Ey1(i,j-1,k))...
            -CB/dy*(Hx1(i,j,k)-Hx1(i,j-1,k))+CB/dx*(Hy(i,j,k)-Hy(i-1,j,k));
d_z2(i,1,k)=d_z2(i,1,k)-a_y*dy/(dy+c0*dt) *(Ez1(i,1,k)+Ez1(i,2,k));
d_z2(i,ny2-1,k)=d_z2(i,ny2-1,k)-c_y*dy/(dy+c0*dt) *(Ez1(i,ny2+1,k)+Ez1(i,ny2,k));

for i=nx1+1:nx2
    for k=nz1:nz2
        j=ny1+1:ny2;
        Ez(i,j,k)=JAy\d_z2(i,j-1,k)';
    end
end
% 
Ez(1,:,:)=-mur_x*Ez(2,:,:)+dx/(dx+c0*dt)*(Ez1(1,:,:)+Ez1(2,:,:));
...
i=nx1:nx2;  j=ny1+1:ny2; k=nz1+1:nz2;
d_x2(i,j,k-1)=Ex1(i,j,k)-CQ*CB/dx/dz*(Ez1(i+1,j,k)-Ez1(i,j,k)-Ez1(i+1,j,k-1)+Ez1(i,j,k-1))...
             -CB/dz*(Hy1(i,j,k)-Hy1(i,j,k-1))+CB/dy*(Hz1(i,j,k)-Hz1(i,j-1,k));
d_x2(i,j,1)= d_x2(i,j,1)-a_z*dz/(dz+c0*dt)*(Ex1(i,j,1)+Ex1(i,j,2));
d_x2(i,j,nz2-1)= d_x2(i,j,nz2-1)-c_z*dz/(dz+c0*dt)*(Ex1(i,j,nz2+1)+Ex1(i,j,nz2));


for i=nx1:nx2
    for j=ny1+1:ny2
        k=nz1+1:nz2;     
        Ex(i,j,k)=JAz\ reshape(d_x2(i,j,:),nz2-1,1);      
    end
end
